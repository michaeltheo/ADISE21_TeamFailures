...
def get_random_board(db: Session = Depends(database.get_db)):
...
def update(id: UUID, request: schemas.Boards, db: Session = Depends(database.get_db)):
    return boards.update(id, request, db)


@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
def destroy(id: UUID, db: Session = Depends(database.get_db)):
